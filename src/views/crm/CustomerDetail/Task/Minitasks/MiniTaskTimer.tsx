...
import { apiGetCrmLeadsSingleMiniTaskDataTimer, apiGetCrmLeadsSingleMiniTaskTimer, apiGetCrmProjectsSingleMiniTaskDataTimer, apiGetCrmProjectsSingleMiniTaskTimer } from '@/services/CrmService';
...
const MiniTaskTimer = ({Data}:any) => {
    ...
    const [verticalOpen, setVerticalOpen] = useState(false)
        ...
        const project_id=queryParam.get('project_id') || ''; 
        ...
        const Submit=async(data:UpdateData)=>{
            const response=await apiGetCrmProjectsSingleMiniTaskTimer(data);
        ...
        const usePersistentTimer = (minitaskId: string) => {
            ...
            useEffect(() => {
                const fetchData = async () => {
                    ...
                    const response = await apiGetCrmProjectsSingleMiniTaskDataTimer(project_id, Data.data.task_id, Data.data.mini_task_id, org_id);
                    if (response) {
  ...
  return (
    <div className={`flex   items-center ${Data?.isShow &&  "gap-4 "}`}>
                      ...
                      {Data.isShow && ((Data.data.mini_task_status==='Completed' || Data.data.mini_task_status==='Cancelled') || Data.data.mini_task_status==='Pending' || ((role !== 'SUPERADMIN' && role !== 'ADMIN') && Data.data.mini_task_assignee !== Data?.users?.find((u:any) => u._id === userId)?.username))?
