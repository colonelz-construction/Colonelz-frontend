...
const Index = () => {
  ...
  const openDialog6 = (secondFolderName: string) => {
    setIsOpen6(true)
    setSecondFolderName(secondFolderName)
  }
    ...
    const fetchDataAndLog = async () => {
      try {
        ...
        const result = data?.flatMap((obj: any) =>
            obj.files
              ?.filter((item: any) => 
                item.folder_name === folderName && item.sub_folder_name_first === sub_folder_name_first
              )
              .flatMap((item: any) => 
                item.sub_folder_name_second 
                  ? [{ sub_folder_name_second: item.sub_folder_name_second, files: item.files || [], total_files: item.files.length, updated_date: item.updated_date }] 
                  : []
              )
          ) || [];
  ...
  // console.log(leadData);
   ...
   const deleteFolder = async (secondFolderName: string) => {
            ...
            const postData = {
                ...
                sub_folder_name_second: secondFolderName,
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <a className="font-medium cursor-pointer" onClick={() => navigate(
                                `/app/crm/fileManager/projects/folder/firstfolder/secondfolder?project_id=${projectId}&project_name=${projectName}&folder_name=Drawing&sub_folder_name_first=${sub_folder_name_first}&sub_folder_name_second=${row.original.sub_folder_name_second}`,
                            )}>
            ...
            cell: ({ row }) => {
                ...
                return (
                    <AuthorityCheck
                        ...
                        authority={role === 'SUPERADMIN' ? ["SUPERADMIN"] : roleData?.data?.file?.delete ?? []}
                    >
                        <div className=' ml-3 cursor-pointer' onClick={() => openDialog6(row.original.sub_folder_name_second)}>
  ...
  return (
    <div>
      <div className='flex justify-between'>
        <h3 className='mb-5'>Project-{projectName}</h3>
              ...
              <div className="flex items-center mb-4">
                <nav className="flex">
                  <ol className="flex items-center space-x-2">
                    ...
                    <li>
                      <Link to={`/app/crm/fileManager/projects?project_id=${projectId}&project_name=${projectName}`} className="text-blue-600 dark:text-blue-400 hover:underline">Leads</Link>
                    </li>
                    <li>
                      <span className="mx-2">/</span>
                    </li>
      ...
      >
        <div className="md:flex items-center">
          ...
          <Button
            ...
            type="button"
            onClick={() => {
              navigate(`/app/crm/fileManager/projects?project_id=${projectId}&project_name=${projectName}`)
            }}
          >
            Back
          </Button>
...
/>
      ...
      >
        ...
        <Formik initialValues={{ project_id: projectId, folder_name: folderName, file_id: '', email: '', cc: '', bcc: '', subject: '', body: '' }}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteFolder(secondFolderName)}
        title="Delete File"
